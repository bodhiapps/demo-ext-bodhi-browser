...
const ChatPage = () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes("500")) {
          setModelsError(
            "Server error while loading models. Please try refreshing the page.",
          );
        } else if (
          ...
          setModelsError(
            "Unable to load models from the remote server. Please try refreshing the page.",
          );
        }
      } else {
        setModelsError(
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: messages and streamingContent are functionally necessary for auto-scroll behavior
  useEffect(() => {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    if (!extension.client) {
      toast({
        ...
        description:
          "Please install and connect the Bodhi browser extension to use chat.",
        variant: "destructive",
        duration: 5000,
      });
      return;
    ...
    if (!modelsLoaded || !availableModels.length || !selectedModel) {
      toast({
        ...
        description:
          "Please wait for models to load or try refreshing the page.",
        variant: "destructive",
        duration: 5000,
      });
      return;
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold">
              Bodhi Browser Extension Demo App
            </h1>
          </div>
          <Button
            variant="ghost"
          ...
          {messages.length === 0 && !isStreaming && !isProcessing ? (
                ...
                {!extension.client ? (
                  <>
                    ...
                    <p className="text-muted-foreground">
                      Please install and connect the Bodhi browser extension to
                      start chatting.
                    </p>
                  </>
                  ...
                  <>
                    ...
                    <p className="text-muted-foreground">
                      Please sign in to start using the chat functionality.
                    </p>
                  </>
                ) : (
                  <>
            ...
            <div className="mx-auto w-full max-w-4xl px-4 py-4">
              {messages
                ...
                .map((message, index) => {
                  // Create a stable key using message content hash and index as fallback
                  const messageKey = `${message.role}-${message.content.substring(0, 50).replace(/\s+/g, "")}-${index}`;
                  return (
                    <div key={messageKey} className="animate-fade-in">
                      <ChatMessage
