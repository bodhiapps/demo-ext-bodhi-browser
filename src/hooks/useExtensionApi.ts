...
export function useExtensionApi(
  extensionClient: ExtensionClient | null,
): UseExtensionApiReturn {
  const { refreshAccessToken, clearTokensAndRedirect, buildAuthHeaders } =
    useAuthServer();

  // Helper function to generate contextual error messages based on endpoint
  const getContextualAuthError = useCallback(
    (endpoint: string, _method: string): string => {
  ...
  const is401Error = useCallback((error: any): boolean => {
    if (error && typeof error === "object") {
      ...
      // ApiResponse format
      if ("body" in error && "status" in error && error.status === 401)
        return true;
      ...
      try {
        ...
        if (is401Error(error)) {
          ...
          if (newToken) {
            ...
            try {
              return await extensionClient.sendApiRequest(
                method,
                endpoint,
                body,
                newHeaders,
              ...
              if (is401Error(retryError)) {
                console.error(
                  "❌ Still getting 401 after token refresh, tokens invalid",
                );
  ...
  const sendStreamRequest = useCallback(
    async (
      ...
      try {
        return await extensionClient.sendStreamRequest(
          method,
          endpoint,
          body,
          headers,
