...
import type { ExtensionClient } from "@/lib/libbodhiext";
...
export function useAuth(
  ...
  const refreshUserInfo = useCallback(async (): Promise<void> => {
    ...
    try {
      const response = await extensionApi.sendApiRequest(
        "GET",
        "/bodhi/v1/user",
      );
      ...
      if (userInfo?.logged_in === false && authServer.isAuthenticated()) {
        console.log(
          "⚠️ User appears logged out on server but we have tokens - attempting token refresh...",
        );
        ...
        try {
          ...
          if (newToken) {
            console.log(
              "✅ Token refreshed successfully, retrying user info...",
            );
            // Retry the user info call with refreshed token
            const retryResponse = await extensionApi.sendApiRequest(
              "GET",
              "/bodhi/v1/user",
            );
            ...
            console.log("📋 Retry user info response:", {
              logged_in: retryUserInfo?.logged_in,
            });

            // If still logged out after refresh, tokens are invalid
            if (retryUserInfo?.logged_in === false) {
              console.error(
                "❌ User still logged out after token refresh - clearing tokens and logging out",
              );
