...
export class ExtensionNotFoundError extends Error {
  constructor(timeout: number) {
    super(
      `Bodhi extension not detected within ${timeout}ms. Please ensure the extension is installed and enabled.`,
    );
...
export class ExtensionTimeoutError extends Error {
  constructor(timeout: number) {
    super(
      `Timeout fetching extension ID within ${timeout}ms. Extension found but not ready.`,
    );
...
export class ExtensionClient {
  ...
  private ensureExtensionAvailable(): void {
    if (typeof window.bodhiext === "undefined") {
      throw new Error(
        "Bodhi extension not available. Extension must be loaded before creating ExtensionClient.",
      );
    }
  }
}
...
export async function loadExtensionClient(
  config: BodhiExtConfig = { timeout: 10000 },
