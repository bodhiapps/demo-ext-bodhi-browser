...
export const BASE_PATH = "/demo-ext-bodhi-browser";

// Function to get redirect URI (client-side only)
export const getRedirectUri = (): string => {
  if (typeof window === "undefined") {
    // Fallback for SSR - this shouldn't be called server-side
    return "http://localhost:3000/demo-ext-bodhi-browser/callback/";
  }
  return `${window.location.origin}${BASE_PATH}/callback/`;
};

// Function to get home URL with base path (client-side only)
export const getHomeUrl = (): string => {
  if (typeof window === "undefined") {
    // Fallback for SSR - this shouldn't be called server-side
    return "http://localhost:3000/demo-ext-bodhi-browser/";
  }
