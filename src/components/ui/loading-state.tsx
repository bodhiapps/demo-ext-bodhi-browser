import { Clock, Loader2, RefreshCw, Wifi } from "lucide-react";
...
export function LoadingState({
  ...
  if (variant === "inline") {
    return (
      <div className={cn("flex items-center gap-2", className)}>
        {showIcon && (
          <Icon
            className={cn(sizeClasses[size], "animate-spin text-gray-400")}
          />
        )}
  ...
  return (
    ...
    >
      {showIcon && (
        <Icon
          className={cn(sizeClasses[size], "mb-3 animate-spin text-gray-400")}
        />
      )}
      <div className="space-y-2">
        <div className={cn("font-medium text-gray-900", textSizeClasses[size])}>
          {displayMessage}
        </div>
        {displayDescription && (
          <p className="max-w-sm text-sm text-gray-600">{displayDescription}</p>
        )}
      </div>
    </div>
  );
...
export function ExtensionLoadingState(
  props: Omit<LoadingStateProps, "variant">,
) {
